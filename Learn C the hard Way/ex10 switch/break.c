...
int main(int argc,char *argv[])
{
	...
	for(i = 0;argv[1][i] != '\0';i++) // the argv is array of strings and each argument is a string and we can obtain the individual character from that string using the subscript operator
	{
		...
		switch(letter)
		{
			...
			case 'A':
				printf("%d 'A'\n",i);
			break; // most common way to break this would be to forget the break statement, now the case after the break will start executing
			...
			default: // also you can forget a default
