...
int main(int argc, char* argv[])
{
	...
	while(1){
		...
		if(FD_ISSET(0, &readmask)){
			...
			check_debug(rc != -1, "Failed to read some from stdin");
		}
		
		if(FD_ISSET(socket, &readmask)){
			rc = read_some(sock_rb, socket, 0);
			check_debug(rc != -1, "Failed to read some from socket");
		}
		
		while(!RingBuffer_empty(sock_rb)){
			rc = write_some(sock_rb, 1, 0);
			check_debug(rc >= 0, "Failed to write to stdout");	
		}
		
		while(!RingBuffer_empty(in_rb)){
			rc = write_some(in_rb, socket, 1);
			check_debug(rc >= 0, "Failed to write to socket");
		}
	}
